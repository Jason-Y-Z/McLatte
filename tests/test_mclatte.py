"""
Note you might experience OSError here due to limitations 
in Pytorch DataLoaders. Try running fewer tests at a time.
"""
import os
import pytest
...
def test_skimmed_mclatte(N, M, H, R, D, K, C, mode, encoder, decoder):
    ...
    trainer = Trainer(
        default_root_dir=os.path.join(os.getcwd(), "tests/data/test_skimmed_mclatte"),
        max_epochs=1,
...
def test_semi_skimmed_mclatte(N, M, H, R, D, K, C, mode, encoder, decoder):
    ...
    trainer = Trainer(
        default_root_dir=os.path.join(
            os.getcwd(), "tests/data/test_semi_skimmed_mclatte"
        ),
        max_epochs=1,
        progress_bar_refresh_rate=0,
        log_every_n_steps=1,
...
def test_mclatte(N, M, H, R, D, K, C, mode, encoder, decoder):
    ...
    trainer = Trainer(
        default_root_dir=os.path.join(os.getcwd(), "tests/data/test_mclatte"),
        max_epochs=1,
        progress_bar_refresh_rate=0,
        log_every_n_steps=1,
    )
