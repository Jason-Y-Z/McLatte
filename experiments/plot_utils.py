...
def get_y_tildes(
    ...
    trained_model = SkimmedMcLatte.load_from_checkpoint(get_ckpt_path('skimmed_mclatte'))
    y_tildes.append((infer_mcespresso(trained_model, *mclatte_args)[1], 'skimmed_mclatte'))
    trained_model = SemiSkimmedMcLatte.load_from_checkpoint(get_ckpt_path('semi_skimmed_mclatte'))
    y_tildes.append((infer_mcespresso(trained_model, *mclatte_args)[2], 'semi_skimmed_mclatte'))
    ...
    y_tildes.append((infer_rnn(trained_model, test_data.y_pre, test_data.y_post, return_Y_pred=True), 'rnn'))
    
    trained_model = SyncTwinPl.load_from_checkpoint(get_ckpt_path('synctwin'))
    y_tildes.append((infer_synctwin(trained_model, test_data.n, test_data.y_post), 'synctwin'))
...
def plot_subject(y_tildes, plot_sub_id, y_pre_plot, post_t, file_suffix, fig):
    for y_tilde, model_name in y_tildes:
        ...
        line_pred_model = go.Scatter(x=post_t, y=y_pred_plot, name=f'{PLOT_NAME[model_name]}{file_suffix}', line={'dash': 'dash'})
        fig.add_trace(line_pred_model)


def plot_config_results(data_name, test_configs, generate_data, config_id, file_suffix='', plot_01_treatment=True):
    ...
    for plot_sub_id in range(test_data.n):
        ...
        if plot_01_treatment:
            ...
            plot_subject(y_tildes, plot_sub_id, y_pre_plot, post_t, file_suffix + ' 01', fig)
        ...
        fig.write_image(f'plots/{data_name}/outcome_pred_{config_id}_{plot_sub_id}{file_suffix}.png')
