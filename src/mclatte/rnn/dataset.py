...
class ShiftingDataset(Dataset):
    ...
    def __init__(self, Y: np.ndarray, seq_len: int) -> None:
        super().__init__()
    ...
    def __getitem__(self, idx):
        ...
        return (
            torch.from_numpy(self._Y[subject_idx, time_start:time_end])
            .unsqueeze(1)
            .float(),
            torch.from_numpy(self._Y[subject_idx, time_start + 1 : time_end + 1])
            .unsqueeze(1)
            .float(),
        )
