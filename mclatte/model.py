...
class McLatte(pl.LightningModule):
    def __init__(
        ...
        lambda_s: float,  # prognosis loss hp
        lr: float,
...
class McLatteSemiSkimmed(pl.LightningModule):
    def __init__(
        self, 
        encoder: nn.Module, 
        decoder: nn.Module, 
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self._lambda_r * recon_loss + self._lambda_d * diag_loss + self._lambda_p * prog_loss
...
class McLatteWhole(McLatteSemiSkimmed):
    def forward(self, x, a, t, mask):
...
def train_mclatte_semi_skimmed(
    config: Dict,
    X,
    M_,
    Y_pre,
    ...
    if test_run > 0:
        # Try loading from checkpoint
        try:
            pl_model = McLatteSemiSkimmed.load_from_checkpoint(os.path.join(os.getcwd(), f'results/mclatte_semi_skimmed_{test_run}.ckpt'))
        except Exception as e:
            print(e)
            pl_model = None
    ...
    trainer.fit(pl_model, data_module)
    if test_run > 0:
        trainer.save_checkpoint(os.path.join(os.getcwd(), f'results/mclatte_semi_skimmed_{test_run}.ckpt'))
    return pl_model


def train_mclatte_whole(
    config: Dict,
    X,
    M_,
    Y_pre,
    ...
    if test_run > 0:
        # Try loading from checkpoint
        try:
            pl_model = McLatteWhole.load_from_checkpoint(os.path.join(os.getcwd(), f'results/mclatte_whole_{test_run}.ckpt'))
        except Exception as e:
            print(e)
            pl_model = None
    ...
    trainer.fit(pl_model, data_module)
    if test_run > 0:
        trainer.save_checkpoint(os.path.join(os.getcwd(), f'results/mclatte_whole_{test_run}.ckpt'))
    return pl_model
